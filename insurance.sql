...
desc ACCIDENT;
create table OWNS(driverid varchar(10),regno varchar(10),primary key(driverid,regno),foreign key(driverid) references PERSON(driverid) on delete cascade,foreign key(regno) references CAR(regno) on delete cascade);
CREATE TABLE PARTICIPATED(driverid varchar(10),regno varchar(10),report_no int,damage_amt float, foreign key (driverid,regno) references OWNS(driverid,regno)ON DELETE CASCADE,foreign key (REPORT_NO) references ACCIDENT(REPORT_NO) ON DELETE CASCADE);
desc PARTICIPATED;
